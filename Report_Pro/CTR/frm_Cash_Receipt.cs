...
namespace Report_Pro.CTR
{
    public partial class frm_Cash_Receipt : frm_Master
    {
        ...
        private void search_1_Click(object sender, EventArgs e)
        {
            ...
            frm.ShowDialog();
            showData(txtStore_ID.Text, frm.txtSearch.t.Text);
        ...
        public override void GoPrevious()
        {
            ...
            showData(txtStore_ID.Text, (txt_sandNo.TextS.ParseInt(0) - 1).ToString());
        ...
        public override void GoLast()
        {
            ...
            showData(txtStore_ID.Text, ser);

            base.GoLast();
        }


        private void showData(string _branch,string _sanad)
        {
            DataTable dt_ = dal.getDataTabl_1(@" select * from (SELECT ACC_YEAR,ACC_NO,BRANCH_code,ser_no,COST_CENTER,meno,g_date,sanad_no,SANAD_TYPE,sp_ser_no
        ...
        public override void Print()
        {
            ...
            dt1 = dal.getDataTabl_1(@"select A.*,B.*,P.PAYER_NAME,P.PAYER_l_NAME,C.branch_name,S.BANK_NAME,S.BANK_NAME_E,P2.PAYER_NAME as db_PAYER_NAME,p2.payer_l_name as db_payer_l_name from (SELECT LEFT(BRANCH_code,1) as com_code,ACC_YEAR,ACC_NO,BRANCH_code,ser_no,COST_CENTER,meno,g_date,sanad_no,SANAD_TYPE,sp_ser_no
                   ...
                   "inner join payer2 as P2 on P2.ACC_NO=A.ACC_NO and P2.BRANCH_code=A.BRANCH_code "+
            ...
            if (dt1.Rows.Count > 0)
            {
                DataTable dt_Co = dal.getDataTabl_1(@"SELECT Company_No,Company_a_name,Company_e_name,User_id,G_date,Comp_Vat_no,Comp_AdressA,Comp_AdressE,Comp_Phone_no,Comp_Fax,Comp_Email,Comp_Web_Site,Comp_Rc
              ,ISNULL(h_pic,'') AS h_pic,ISNULL(f_pic,'') AS f_pic,ISNULL(stamp_pic,'') AS stamp_pic,PO_Box FROM main_acc_wh.dbo.Wh_Oiner_Comp where Company_No='" + dt1.Rows[0]["com_code"] + "'  ");
                ...
                if (Properties.Settings.Default.lungh == "0")
                {
                    ...
                    rpt.txtDepositAcc.Text = dt1.Rows[0]["db_PAYER_NAME"].ToString();
                }
                else
                {
                    ...
                    rpt.txtDepositAcc.Text = dt1.Rows[0]["db_payer_l_name"].ToString();
                }
                ...
                if (Encoding.ASCII.GetString((Byte[])dt_Co.Rows[0]["stamp_pic"]) != string.Empty)
                ...
                if (Encoding.ASCII.GetString((Byte[])dt_Co.Rows[0]["h_pic"]) != string.Empty)
                {
        ...
        private void simpleButton2_Click(object sender, EventArgs e)
        {
            if (Account.ID.Text.Trim() != string.Empty)
            {
                DataTable dt_inv = dal.getDataTabl_1(@"select LEFT(X.BRANCH_code,1) as com_code,x.Ser_no,x.Cyear,x.Branch_code,x.Transaction_code,x.G_date,x.Acc_no,x.Acc_Branch_code,x.Payment_Type
                ,x.Inv_no,x.Inv_date,x.Po_no,x.Inv_Notes,x.NetAmount,x.K_M_Credit_TAX,B.paidAmount
                ,x.InvoiceAmount,isnull(y.returnAmount,0) as returnAmount,(x.InvoiceAmount+isnull(y.returnAmount,0) - isnull(B.paidAmount,0)) as Balance
                ...
                DataTable Head_Footer = dal.getDataTabl_1(@"SELECT Company_No, h_pic, f_pic, stamp_pic FROM Wh_Oiner_Comp where Company_No =left('" + Properties.Settings.Default.BranchId+ "',1) ");
                DataTable dt_Co = dal.getDataTabl_1(@"select * from Wh_Oiner_Comp where Company_No='" + dt_inv.Rows[0]["com_code"] + "'  ");

                DataSet ds = new DataSet();
                ds.Tables.Add(dt_inv);
                ds.Tables.Add(Head_Footer);
                ds.Tables.Add(dt_Co);
                ds.WriteXmlSchema("schemaInv.xml");
                RPT.rpt_NonPaidInvoices rpt = new RPT.rpt_NonPaidInvoices();
                rpt.DataDefinition.FormulaFields["digits_"].Text = dal.digits_.ToString();
        ...
        {
            if (ChAllAccount.Checked)
            {
                Account.txtMainAcc.Text = "";
            }
            else
            {
                Account.txtMainAcc.Text = dt_Bdata.Rows[0]["Costmers_acc_no"].ToString();
