...
{
    public partial class frm_payment_voucher : frm_Master
    {
        ...
        .ToList().ForEach(x =>
        {
            ...
            x.BOX_SARF_SER = Convert.ToInt32(txt_sandNo.TextS);
        });
        ...
        private void showData(string _branch,string _sanad)
        {
            ...
            ,Dafter_no,Dafter_ser,SANAD_TYPE2,desc_E  FROM daily_transaction where SANAD_TYPE2 in('CE') and BRANCH_code='" + _branch + "' and  sanad_no ='" + _sanad + "' and  meno>0) as A " +
            ", (select acc_no as acc_cr,desc2 as desc_cr   from daily_transaction where SANAD_TYPE2 in('CE') and BRANCH_code='" + _branch + "' and  sanad_no ='" + _sanad + "' and loh>0)  as cr_acc ");
        ...
        private void search_2_Click(object sender, EventArgs e)
        {
            ...
            DataTable dt_ = dal.getDataTabl_1(@" select * from (SELECT ACC_YEAR,ACC_NO,BRANCH_code,ser_no,COST_CENTER,meno,g_date,sanad_no,SANAD_TYPE,sp_ser_no
            ,user_name,desc2,sheek_no,sheek_bank,sheek_date,sheek_or_cash,notes,SOURCE_CODE,Wh_Branch_Code,Sheek,payType_No,shequeBank_no
            ,Dafter_no,Dafter_ser,SANAD_TYPE2,desc_E  FROM daily_transaction where SANAD_TYPE2 in('CE') and BRANCH_code='" + txtStore_ID.Text + "' and  ser_no ='" + frm.txtSearch.t.Text + "' and  meno>0) as A " +
            ", (select acc_no as acc_cr,desc2 as desc_cr   from daily_transaction where SANAD_TYPE2 in('CE') and BRANCH_code='" + txtStore_ID.Text + "' and  ser_no ='" + frm.txtSearch.t.Text + "' and loh>0)  as cr_acc ");
