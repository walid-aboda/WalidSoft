...
namespace Report_Pro.PL
{
    ...
    public partial class frmDelivryNote : frm_Master
    {
        ...
        public override void Attachment()
        {
            ...
            DataTable dt_ = dal.getDataTabl_1(@"select ser_no,Transaction_code from wh_inv_data_Delevry where Ser_no= '" + txt_InvNu.Text + "' and Cyear ='" + year_.Text + "' and Transaction_code = '" + Doc_Type.Text + "' and Branch_code = '" + txtStore_ID.Text + "'");
            if (dt_.Rows.Count > 0)
            {
...
//                     + "' and Cyear= '"+year_.Text+"'";
        ...
        private void showDelevery(string serNo_, string brancCode_,string transaction_, string year_)
        {
            getDelevery(serNo_, brancCode_, transaction_, year_);
            if (dt_Q.Rows.Count > 0)
            {
                ...
                if (rowscount > 0)
                {
                    ...
                    for (int i = 0; i <= (rowscount - 1); i++)
                    {
                        ...
                        invGrid1.dgv1.Rows[i].Cells["_stockType"].Value = dt_Q.Rows[i]["NonStockItem"].ToString();

                        if (invGrid1.dgv1.Rows[i].Cells["_stockType"].Value.ToString() == "False")
                        {
                        invGrid1.dgv1.Rows[i].Cells["_oldBalance"].Value = (dt_Q.Rows[i]["BALANCE"].ToString().ToDecimal() + dt_Q.Rows[i]["QTY_TAKE"].ToString().ToDecimal()).ToString("N2");
                        invGrid1.dgv1.Rows[i].Cells["_balance"].Value = (dt_Q.Rows[i]["BALANCE"].ToString().ToDecimal() - dt_Q.Rows[i]["QTY_TAKE"].ToString().ToDecimal()).ToString("N2");
                        }
                        else
                        {
                            invGrid1.dgv1.Rows[i].Cells["_oldBalance"].Value = "0";
                            invGrid1.dgv1.Rows[i].Cells["_balance"].Value = "0";
                                                                               
                        }
                        invGrid1.dgv1.Rows[i].Cells["_cost"].Value = dt_Q.Rows[i]["local_cost"].ToString().ToDecimal().ToString("N" + dal.digits_);
                        invGrid1.dgv1.Rows[i].Cells["_mainBalance"].Value = (dt_Q.Rows[i]["MainBalance"].ToString().ToDecimal()+ dt_Q.Rows[i]["QTY_TAKE"].ToString().ToDecimal()).ToString("N2"); ;
                ...
                if (txtInvNo.Text.Trim() != "")
                {
                    ...
                    btnUpdateDeliveryPrice.Enabled = false;
                }
                else
                {
                    tabControl1.Enabled = true;
                    invGrid1.Enabled = true;
                    pictureBox1.Visible = false;
                    btnUpdateDeliveryPrice.Enabled = true;
                }
        ...
        public override void GoNext()
        {
            ...
            string MyResult = "";
            dtNaf = dal.getDataTabl_1(@"SELECT DISTINCT ser_no  FROM wh_inv_data_Delevry
            ...
            if (RowNo < dtNaf.Rows.Count - 1 && RowNo != -1)
            {
               ...
               showDelevery(MyResult, txtStore_ID.Text, Doc_Type.Text, Properties.Settings.Default.C_year);
            }
        ...
        {
            showDelevery(txtSearch.Text,txtStore_ID.Text, Doc_Type.Text,Properties.Settings.Default.C_year);
            // getDelevery(txtSearch.Text, txtStore_ID.Text, Doc_Type.Text,Properties.Settings.Default.C_year);
            //if (dt_Q.Rows.Count > 0)
            //{
            //    txt_InvNu.Text = dt_Q.Rows[0]["Ser_no"].ToString();
            //    txt_InvDate.Text = dt_Q.Rows[0]["G_date"].ToString();
        ...
        private void getDelevery(string ser_, string branch_, string transaction_, string cyear_)
        {
            ...
            { //, ReportDB.dbo.Tafkeet('+@4+', '''+@5+''')  from wh_Po_Cot_inv_data As A
                ...
                B.main_counter, B.ITEM_NO,M.descr,M.Descr_eng,M.NonStockItem,B.DETAILS,B.Unit,M.Weight,B.QTY_TAKE,B.Local_Price,B.TAX_OUT
            ,isnull((select sum(QTY_Balance+Delever_BALANCE) as baance from VIEW_balance as V  where  V.item_no = B.item_no and branch_code like A.Branch_code+'%' ),0) as balance 
            ,isnull((select sum(S.QTY_TAKE - S.QTY_ACT) as MBalance from VIEW_Get_Sale_order as S  where S.Ser_no = A.SO_no and S.ITEM_NO =  B.ITEM_NO and S.Branch_code = A.Branch_code ),0) as MainBalance 
             ,B.Pice_Total_Cost,B.K_M_TYPE_ITEMS,K.KM_RATIO,B.total_disc,B.DISC_R,B.DISC_R2,B.DISC_R3,M.local_cost,
        ...
        {
            //DataTable dt_AllQ = dal.getDataTabl_1(@"SELECT A.Ser_no,A.Branch_code,A.Transaction_code,A.Cyear
            //,A.G_date,A.Acc_no,A.Acc_Branch_code,A.Payment_Type,A.Sales_man_Id
            ...
            //where a.Transaction_code ='SO' And A.Branch_code = '" + _branch + "' and(B.PAYER_NAME like '%" + _accName + "%' Or  B.payer_l_name like '%" + _accName + "%') and A.Ser_no like case when '" + _ser.Trim() + "'='' then '%' else '" + _ser + "' end ");

            //DataTable dt_AllQ = dal.getDataTabl_1(@"SELECT DISTINCT  A.ser_no ,A.Branch_code,A.G_date,A.Cyear,A.Transaction_code,B.item_no,B.QTY_TAKE
            //        ,(SELECT SUM(B1.QTY_TAKE) AS Expr1
            //         FROM wh_inv_data_Delevry As A1 
            //        INNER JOIN   wh_MATERIAL_TRANSACTION_Delevry as B1 
            //        on A1.Branch_code = B1.Branch_code and  A1.Cyear = B1.Cyear and A1.TRANSACTION_CODE = B1.TRANSACTION_CODE and A1.Ser_no = B1.SER_NO
            //        WHERE A.Ser_no = A1.QUOT_REF_NO AND A.Branch_code = A1.Branch_code AND B.ITEM_NO = B1.ITEM_NO) AS QTY_ACT

            //        from wh_SaleOrder_inv_data as A
            //        inner join wh_SaleOrder_MATERIAL_TRANSACTION B
            //        on A.Branch_code = B.Branch_c     ode and  A.Cyear = B.Cyear and A.TRANSACTION_CODE = B.TRANSACTION_CODE and A.Ser_no = B.SER_NO
            //        inner join payer2 as C on A.Acc_no = C.ACC_NO and A.Acc_Branch_code = C.BRANCH_code
            //        where A.ser_no =1
            //        group by A.ser_no ,A.Branch_code,A.G_date,A.Cyear,A.Transaction_code,B.item_no,B.QTY_TAKE");


            DataTable dt_AllQ = dal.getDataTabl_1(@"SELECT * FROM VIEW_Get_all_Sale_order AS a
                 where A.Ser_no like '" + _ser + "%'  and A.Branch_code Like '" + _branch + "%'  and A.transaction_code = 'so'  and A.cyear = '" + Properties.Settings.Default.C_year +
                 "'  and A.QTY_TAKE - A.QTY_ACT > 0 and Po_Status ='1' ");
            ...
            if (rowscount > 0)
            {
...

                {
                    ...
                    dgvQuotations.Rows[i].Cells[_soNo.Name].Value = dt_AllQ.Rows[i]["ser_no"].ToString();
                    dgvQuotations.Rows[i].Cells[_soDate.Name].Value = Convert.ToDateTime(dt_AllQ.Rows[i]["G_date"]).ToString("dd/MM/yyyy");
            ...
            if (dt_Q.Rows.Count > 0)
            {
                ...
                if (rowscount > 0)
                {
...

                    {
                        ...
                        invGrid1.dgv1.Rows[i].Cells["_stockType"].Value = dt_Q.Rows[i]["NonStockItem"].ToString();
                        if (dt_Q.Rows[i]["NonStockItem"].ToString() == "False")
                        {
        ...
        private void UpdateDeliveryPrice_Click(object sender, EventArgs e)
        {

            //DataTable dt_Q = dal.getDataTabl_1(@"SELECT * FROM VIEW_Get_Sale_order AS a  where A.Ser_no = '" + txtSoNo.Text + "'  and A.Branch_code = '" + txtStore_ID.Text + "'  and A.transaction_code = 'SO'  and A.cyear = '" + year_.Text + "' and ITEM_NO='" + invGrid1.dgv1.ro + "  order by A.main_counter ");
            ...
            //    {
            //        dal.Execute_1(@"update wh_MATERIAL_TRANSACTION_Delevry set Local_Price ='"+ dt_Q.Rows[i]["Local_Price"].ToString().ToDecimal().ToString("N" + dal.digits_)+
            //            "', TAX_OUT ='" + dt_Q.Rows[i]["TAX_OUT"].ToString().ToDecimal().ToString("N" + dal.digits_) + "' where SER_NO='"+ txt_InvNu.Text+
            //            "' and TRANSACTION_CODE='"+Doc_Type.Text+ "' and Cyear='"+ year_.Text+ "' and Branch_code='"+ txtStore_ID.Text+"' and ITEM_NO='" + dt_Q.Rows[i]["ITEM_NO"].ToString()+"'");
