...
{
    public partial class frm_Preforma_Internal : frm_Master
    {
        ...
        public override void Save()
        {
            ...
            else
            {
                ...
                "','" + Authorized_Persson.ID.Text +"','"+txtQty.Text.ToDecimal()+"','')";
                    ...
                    cmd.CommandText = @"UPDATE  wh_Serial SET  PEQ= '" + txtId.Text + "' WHERE Branch_code = '" + txtBranch.ID.Text + "' and Cyear='" + txtDate.Value.ToString("yy") + "' ";
                    cmd.ExecuteNonQuery();
        ...
        private void printTons()
        {
            ...
            get_data(txtId.Text, txtBranch.ID.Text, comboBox1.Text.Substring(comboBox1.Text.Length - 2), Doc_Type.Text);
            ...
            //if (Properties.Settings.Default.lungh == "0")
            //{
            //    rpt_P.DataDefinition.FormulaFields["doc_title"].Text = "'عقد مشتريات محلي'";
            //}
            //else
            //{
                rpt_P.DataDefinition.FormulaFields["doc_title"].Text = "'Local Purchase Contract'";
           // }
        ...
        private void get_invSer()
        {
            //try
            //{
            DataTable dtSer = dal.getDataTabl_1(@"select isnull(PEQ+1,1) from wh_Serial where Branch_code= '" + txtBranch.ID.Text + "' and Cyear='" + txtcyear.Text + "'");
        ...
        private void updateSer()
        {
            ...
            if (dtSer.Rows.Count > 0)
            {
                dal.Execute_1(@"UPDATE  wh_Serial SET  PEQ= '" + txtId.Text + "' WHERE Branch_code = '" + txtBranch.ID.Text + "' and Cyear='" + txtDate.Value.ToString("yy") + "' ");

            }
            else
            {
                dal.Execute_1(@"INSERT INTO wh_Serial (Branch_code,Cyear,PEQ) Values('" + txtBranch.ID.Text + "' ,'" + txtDate.Value.ToString("yy") + "','" + txtId.Text + "')");
