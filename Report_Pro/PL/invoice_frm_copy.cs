...
{
    public partial class invoice_frm_copy : frm_Master
    {
        string btntype = "0";
        Int32 m;
        //string Jor_no;
        ...
        private void search_Inv(string _transaction)
        {
            ...
            if (dt_inv.Rows.Count > 0)
            {
              ...
              //  Uc_Project.ID.Text = dt_inv.Rows[0]["PROJECT_NO"].ToString();
                ...
                invGrid1.txt_downPayment.Text = dt_inv.Rows[0]["Dowen_Payment"].ToString().ToDecimal().ToString("N"+dal.digits_);
                txtProject.ID.Text = dt_inv.Rows[0]["PROJECT_NO"].ToString();
...
getSalesInv(Sh_ser, txtStore_ID.Text, sh_transaction, Properties.Settings.Default.C_year);
            ...
            private void invoice_frm_Load(object sender, EventArgs e)
        {
            ...
            btnGoFrist.Visible = false;
            btnGoPrevious.Visible = false;
            btnGoNext.Visible = false;
            btn_GoLast.Visible = false;
        ...
        public override void Print()
        {
           ...
           if (Program.PrintMode == "1") //english
            ...
            else if (Program.PrintMode == "4") // both

            {
                RPT.Rpt_Contract_Invoice reportInv = new RPT.Rpt_Contract_Invoice();
                reportInv.SetDataSource(ds);
                frminv.crystalReportViewer1.ReportSource = reportInv;
                frminv.ShowDialog();
            }
        ...
        private void txt_DP_Ratio_TextChanged(object sender, EventArgs e)
        {
           invGrid1.txt_downPayment_Ratio.Text = txt_DP_Ratio.Text.ToDecimal().ToString();
            invGrid1.total_inv();
        }

        private void txtProject_Load(object sender, EventArgs e)
        {
            DataTable dt_ratio = dal.getDataTabl_1(@"select PROJ_CODE,DP_Ratio,DP_Acc  from Projects where PROJ_CODE = '" + txtProject.ID.Text + "'");
            if (dt_ratio.Rows.Count > 0)
            {
                txt_DP_Ratio.Text = dt_ratio.Rows[0]["DP_Ratio"].ToString().ToDecimal().ToString("N0");
                 txt_downPayment_Acc.ID.Text = dt_ratio.Rows[0]["DP_Acc"].ToString();

            }
            else
            {
                txt_DP_Ratio.Text = "0".ToDecimal().ToString("N0");
                txt_downPayment_Acc.ID.Text = "";
            }
 ...
 + "' and ACC_YEAR= '" + acc_year.Text + "' ");//.PadLeft(4, '0');
        ...
        private void add_wh_SaleOrder_inv_data()
        {
            ...
            K_M_ACC_NO, K_M_Credit_TAX,K_M_Debit_TAX, COSTMER_K_M_NO, KM_CODE_ACC, MAIN_KM_CODE, Inv_Notes, OPEN_VAT,
            VAT_RATIO,QUOT_REF_NO,QUOT_REF_DATE,SO_no,acc_serial_no,NetAmount,Dowen_Payment_acc_no,Dowen_Payment,Dowen_Payment_Vat,Sales_Vat,Dowen_Payment_Ratio,PROJECT_NO)
            values('" + txt_InvNu.Text +
                ...
                "','" + txt_downPayment_Acc.ID.Text + 
                "','" + invGrid1.txt_downPayment.Text.ToDecimal() +
                "','" + invGrid1.txt_downPaymentVAT.Text.ToDecimal() + 
                "','" + invGrid1.txtTotalVAT.Text.ToDecimal() + 
                "','" + txt_DP_Ratio.Text.ToDecimal() + 
                "','"+txtProject.ID.Text +"' )";
                cmd.ExecuteNonQuery();
