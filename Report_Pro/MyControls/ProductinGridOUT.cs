...
{
    public partial class ProductinGridOUT : UserControl
    {
       ...
       public bool qtyOnly;
        ...
        public void total_inv()
        {
          ...
          //  {
          //      txtRawKGCost.Text = (txt_SubTot.Text.ToDecimal() / totalWeight.Text.ToDecimal()).ToString();
          //  }
          //  else
          //  {
          //      txtRawKGCost.Text = "0";
       ...
       // }
        ...
        private void get_ItemData(int _rowIndex,string item_No)
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                dgv1.Rows[_rowIndex].Cells[_cost.Name].Value = (txtRawKGCost.Text.ToDecimal() * dt.Rows[0]["Weight"].ToString().ToDecimal()).ToString("N3");
                dgv1.Rows[_rowIndex].Cells[_oldCost.Name].Value = dt.Rows[0]["local_cost"].ToString().ToDecimal().ToString("N3");
                dgv1.Rows[_rowIndex].Cells[_oldBalance.Name].Value = dt.Rows[0]["BALANCE"].ToString();
                dgv1.Rows[_rowIndex].Cells[_balance.Name].Value = (dt.Rows[0]["BALANCE"].ToString().ToDecimal()).ToString("N0");
            ...
            else
            {
                ...
                if (frm.clos_ == 1)
                {
                    if (frm.uc_SearchItem1.dGV_pro_list.SelectedRows.Count > 0)
                    {
                        ...
                        dgv1.CurrentCell = dgv1[dgv1.Columns[_qty.Name].Index, _rowIndex];
        ...
        private void get_ItemData_Barcode(int _rowIndex, string _barcode)
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                dgv1.Rows[_rowIndex].Cells[_oldCost.Name].Value = dt.Rows[0]["local_cost"].ToString().ToDecimal().ToString("N3");
                dgv1.Rows[_rowIndex].Cells[_oldBalance.Name].Value = (dt_coil.Rows[0]["coile_balance"].ToString().ToDecimal()).ToString("N0");
                dgv1.Rows[_rowIndex].Cells[_balance.Name].Value = (dt_coil.Rows[0]["coile_balance"].ToString().ToDecimal()).ToString("N0");
                clculat_amount(_rowIndex);
        ...
        private void get_ItemData_2(int _rowIndex, string item_No)
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                dgv1.Rows[_rowIndex].Cells[_oldBalance.Name].Value = dt.Rows[0]["BALANCE"].ToString().ToDecimal().ToString("N0");
                dgv1.Rows[_rowIndex].Cells["_cost"].Value = (txtRawKGCost.Text.ToDecimal()* dt.Rows[0]["Weight"].ToString().ToDecimal()).ToString("N3");
                dgv1.Rows[_rowIndex].Cells[_oldCost.Name].Value = dt.Rows[0]["local_cost"].ToString().ToDecimal().ToString("N3");
        ...
        public void clculat_amount(int _rowIndex)
        {
            try
            {
                ...
                double qty_, cost_, _old_cost, _unitWeight,_old_balance, _main_Balance;
            ...
            else
            {
                ...
                double itemTotalAmount = (from DataGridViewRow row in dgv1.Rows
                                          where row.Cells["_total"].FormattedValue.ToString() != string.Empty && row.Index <= _rowIndex && row.Cells["_code"].FormattedValue.ToString() == dgv1.Rows[_rowIndex].Cells["_code"].FormattedValue.ToString()
                                          select Convert.ToDouble(row.Cells["_total"].FormattedValue)).Sum();



                dgv1.Rows[_rowIndex].Cells["_balance"].Value = (_old_balance + itemTotalQty).ToString("N0");




                dgv1.Rows[_rowIndex].Cells["_newCost"].Value = Math.Round(((_old_balance * _old_cost) + itemTotalAmount) / (_old_balance + itemTotalQty), 4);
        ...
        private void dgv1_CellEndEdit(object sender, DataGridViewCellEventArgs e)
        {
            ...
            try
            {
                ...
                if (j == dgv1.Columns[_code.Name].Index)
                {
                    ...
                    SendKeys.Send("{up}");
                    dgv1.CurrentCell = dgv1[dgv1.Columns[_qty.Name].Index, i];
                ...
                else if (j == dgv1.Columns[_barcodeNo.Name].Index)
                {
                    ...
                    if (dt_coil.Rows.Count > 0)
                    {
                        ...
                        get_ItemData_Barcode(i, _barcode);
